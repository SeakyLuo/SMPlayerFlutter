...
仅在用户明确要求“参考 Electron / 原版 / 对齐 / parity / 迁移 / 一模一样”等目标时适用。未触发这些目标时，不要把 Electron 当成当前改动的默认标准。

## Electron 对齐触发条件

只有当用户明确要求“参考 Electron / 原版 / 对齐 / parity / 迁移 / 一模一样”等目标时，Electron 才是该任务的 truth source。
...
Electron 是该对齐任务的 truth source；除非用户明确要求改变行为，否则 Flutter 必须复制 Electron 的 UI、交互和业务逻辑。
...
任何“参考 Electron / 对齐原版 / 一模一样 / parity / 迁移”的任务，先审计，后实现。审计必须覆盖：
...
Electron 对齐任务只有同时满足以下条件，才能报告完成：
...
1. 对齐 Electron 的任务中，类名和原版尽可能保持一致。
